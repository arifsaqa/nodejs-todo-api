...
export const addTodo = async (req: Request, res: Response): Promise<void> => {
  ...
  const updatedTodosAfterSave = await TodoModel.find();

  res.status(201).json({
    message: 'Todo successfully added!',
    addedTodo: newTodo,
    allTodosAfterAddition: updatedTodosAfterSave,
  });
};
...
export const updateTodo = async (
  ...
  if (!updatedTodo) {
    res.status(501).json({
      status: 501,
      errorMessage: 'Edit todo is failed, not implemented',
    });
    return;
  }
  res.status(200).json({
...
export const removeTodo = async (
  ...
  if (!removedTodo) {
    res.status(501).json({
      status: 501,
      errorMessage: 'Delete todo is failed, not implemented',
    });
    return;
  }
  res.status(200).json({
